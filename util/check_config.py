...
def extract_config(path, key):
    with open(path, "r") as f:
        config = json.load(f)
        if key == "CHSTR" and config["PGPIN"]["value"] == "BPR":
            return config["CHSTR"]["value"] - 1
        elif key == "M3_Offset":
            # align string length to 2
            if config["M3_Offset"]["value"] // 2 < 10:
                return "0" + str(config["M3_Offset"]["value"] // 2)
            return config[key]["value"] // 2
        elif key == "SD_PASSTHROUGH":
            if config["SD_PASSTHROUGH"]["value"] == 0:
                return "SDCX"
            else:
                return "SDPT"
        elif key == "GATE_PASSTHROUGH":
            if config["GATE_PASSTHROUGH"]["value"] == 0:
                return "GCX"
            else:
                return "GPT"
        else:
