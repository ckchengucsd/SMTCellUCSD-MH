    ...
    def GetMacroGdtStr(conv, cellName, techInfo, isUseMaxCellWidth, macroPinInfo):
        ...
        for inst in insts:
            ...
            if techInfo.multi_height_mode:
                ...
                if ly_to_mos_type[inst.ly] == "PMOS" and ly_to_cell_row[inst.ly] == 0:
                    # PMOS @ row 0 (top)
                    lyActive = yOffset_Active + row_height + 0.001
        ...
        for inst in insts:
            ...
            for i in range(xInst, xInst + inst.totalWidth, 2):
                isPgContact = 1
                if int(inst.ly) == 1 and (
                    (2, i) in pinCols_PMOS):
                    isPgContact = 0
                elif int(inst.ly) == 2 and (
                    (1, i) in pinCols_PMOS):
                    isPgContact = 0
        ...
        for col in sortedColListNMOS:
            ...
            if col[2]:  # isPgContact
                ...
                if techInfo.numTrack == 6:
                    ...
                    print("ERROR: numTrack is not supported -- 1")
                    exit(1)
            else:  # not PgContact
                if col[4] == nmos_lower_plc:
                    lyLisd = (col[4]) * finPitch
                ...
                if techInfo.numTrack == 6:
                    ...
                    print("ERROR: numTrack is not supported -- 2")
                    exit(1)
            # uyLisd = lyLisd + finPitch*(col[1]+col[2]+col[3]+0.25)
            lxLisd = xCoordLisd - lisdWidth / 2
            uxLisd = xCoordLisd + lisdWidth / 2
        ...
        for col in sortedColListPMOS:
            ...
            if col[2]:  # isPgContact
                ...
                if col[4] == pmos_lower_plc:
                    uyLisd = col[4] * finPitch + 2 * finPitch + 0.024
                if col[4] == pmos_upper_plc:
                    uyLisd = col[4] * finPitch + 4 * finPitch - 0.006
                if techInfo.numTrack == 6:
                    if techInfo.bprFlag == BprMode.BPR:
                    ...
                    print("ERROR: numTrack is not supported -- 3")
                    exit(1)
            else:  # not PgContact
                if col[4] == pmos_lower_plc:
                    uyLisd = col[4] * finPitch + 3 * finPitch - 0.02
                if col[4] == pmos_upper_plc:
                    uyLisd = col[4] * finPitch + 4 * finPitch - 0.004
                if techInfo.numTrack == 6:
                    ...
                    print("ERROR: numTrack is not supported -- 4")
                    exit(1)
            # lyLisd = uyLisd - finPitch*(col[1]+col[2]+col[3]+0.25)
            lxLisd = xCoordLisd - lisdWidth / 2
            uxLisd = xCoordLisd + lisdWidth / 2
        ...
        for col in sortedColListNMOS:
            ...
            if techInfo.numTrack == 6:
                ...
                print("ERROR: numTrack is not supported -- 5")
        ...
        for col in sortedColListPMOS:
            ...
            if col[4] == pmos_upper_plc:
                uyLisd = col[4] * finPitch + 4 * finPitch - 0.006
            if col[4] == pmos_lower_plc:
                uyLisd = col[4] * finPitch + 2 * finPitch
            if techInfo.numTrack == 6:
                if techInfo.bprFlag == BprMode.BPR:
                ...
                print("ERROR: numTrack is not supported -- 6")
