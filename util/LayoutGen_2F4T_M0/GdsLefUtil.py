...
class Utility(object):
    ...
    def GenerateLef(inputDir, inputFileList, outputDir, techInfo):
        ...
        for curFile in inputFileList:
            ...
            # curRpaStr, curMacroPinInfo = Utility.GetMacroRPAStr(
            #     cont, curFile[:-5], techInfo
            # )
    ...
    def GetMacroGdtStr(conv, cellName, techInfo, isUseMaxCellWidth, macroPinInfo):
        ...
        for inst in insts:
            ...
            if techInfo.bprFlag == BprMode.BPR:
                yOffset_Active = (
                    techInfo.metal0Pitch / 4 / 1000.0 + finPitch / 2 - 0.001
                )
            else:
            ...
            if techInfo.multi_height_mode:
                ...
                single_row_height = (techInfo.numTrack - 2) // 4
                for i in range(4):
                    ly_to_mos_type[i * single_row_height] = placement_order[i]
                    # check if it is the first row
                    if i > 1:
                        ly_to_cell_row[i * single_row_height] = 1
                    else:
                        ly_to_cell_row[i * single_row_height] = 0

                row_height = techInfo.cellHeight / 1000.0 / 4
                if ly_to_mos_type[inst.ly] == "PMOS" and ly_to_cell_row[inst.ly] == 0:
                    # PMOS @ row 0 (top)
                    lyActive = yOffset_Active + row_height + 0.002
        ...
        # print("************************ PINS ************************")
        for pin in sorted(pins):
            # if techInfo.placement_row_to_cell_row[pin.row] == "PMOS_0":
            #     pinCols_PMOS_bottom.append(int(pin.col / techInfo.cp_factor))
            # elif techInfo.placement_row_to_cell_row[pin.row] == "PMOS_1":
            #     pinCols_PMOS_top.append(int(pin.col / techInfo.cp_factor))
            # elif techInfo.placement_row_to_cell_row[pin.row] == "NMOS_0":
            #     pinCols_NMOS_bottom.append(int(pin.col / techInfo.cp_factor))
            # elif techInfo.placement_row_to_cell_row[pin.row] == "NMOS_1":
            #     pinCols_NMOS_top.append(int(pin.col / techInfo.cp_factor))
            if techInfo.numTrack == 6:
                # NPPN
                if pin.row in [1, 2]:
                ...
                if pin.row in [0, 5]:
                    # print("NMOS pin", pin.row, pin.col)
                    pinCols_NMOS.append((pin.row, int(pin.col / techInfo.cp_factor)))
            elif techInfo.numTrack == 10:
                # NPPN
                if pin.row in [2, 3, 4, 5]:
                    # print("PMOS pin", pin.row, pin.col)
                    pinCols_PMOS.append((pin.row, int(pin.col / techInfo.cp_factor)))
                if pin.row in [0, 1, 6, 7]:
                    # print("NMOS pin", pin.row, pin.col)
                    pinCols_NMOS.append((pin.row, int(pin.col / techInfo.cp_factor)))
            elif techInfo.numTrack == 14:
                # NPPN
                if pin.row in [3, 4, 5, 6, 7, 8]:
                    # print("PMOS pin", pin.row, pin.col)
                    pinCols_PMOS.append((pin.row, int(pin.col / techInfo.cp_factor)))
                if pin.row in [0, 1, 2, 9, 10, 11]:
                    # print("NMOS pin", pin.row, pin.col)
                    pinCols_NMOS.append((pin.row, int(pin.col / techInfo.cp_factor)))
            else:
        ...
        for inst in insts:
            ...
            for i in range(xInst, xInst + inst.totalWidth, 2):
                ...
                if int(inst.ly) == 2 and (
                    (4, i) in pinCols_PMOS or (5, i) in pinCols_PMOS
                ):
                    isPgContact = 0
                elif int(inst.ly) == 4 and (
                    (2, i) in pinCols_PMOS or (3, i) in pinCols_PMOS
                ):
                    isPgContact = 0
                elif int(inst.ly) == 0 and (
                    (6, i) in pinCols_NMOS or (7, i) in pinCols_NMOS
                ):
                    isPgContact = 0
                elif int(inst.ly) == 6 and (
                    (0, i) in pinCols_NMOS or (1, i) in pinCols_NMOS
                ):
                    isPgContact = 0
                # isExtCol
                isExtCol = 0
        ...
        for col in sortedColListNMOS:
            ...
            if col[2]:  # isPgContact
                ...
                if col[4] == nmos_upper_plc:
                    lyLisd = (col[4]) * finPitch + finPitch * 3 + 0.006
                if techInfo.numTrack == 6:
                    if techInfo.bprFlag == BprMode.BPR:
                    ...
                    if techInfo.bprFlag == BprMode.BPR:
                        ...
                        uyLisd = lyLisd + finPitch * (col[1] + col[2] + col[3] - 0.75)
                elif techInfo.numTrack == 10 or techInfo.numTrack == 12:
                    if techInfo.bprFlag == BprMode.BPR:
                        uyLisd = lyLisd + finPitch * (col[1] + col[2] + col[3] - 0.75)
                    else:
                        uyLisd = lyLisd + finPitch * (col[1] + col[2] + col[3] - 0.25)
                elif techInfo.numTrack == 11:
                    # RT == 5
                    if techInfo.bprFlag == BprMode.BPR:
                        uyLisd = lyLisd + finPitch * (col[1] + col[2] + col[3] - 1.25)
                    else:
                ...
                if col[4] == nmos_lower_plc:
                    lyLisd = (col[4]) * finPitch
                if col[4] == nmos_upper_plc:
                    lyLisd = (col[4]) * finPitch + finPitch * 2 + 0.008
                if techInfo.bprFlag == BprMode.BPR:
        ...
        for col in sortedColListPMOS:
            # col[4] is inst.ly
            if col[4] == 4 and (col[0], 0) in LISD_connected_columns:
                continue
            if col[4] == 2 and (col[0], 1) in LISD_connected_columns:
                continue
            xCoordLisd = xOffset + col[0] * gatePitch / 2
            ...
            if col[2]:  # isPgContact
                # uyLisd = uy
                if col[4] == 2:
                    uyLisd = col[4] * finPitch + 3 * finPitch + 0.024
                if col[4] == 4:
                    uyLisd = col[4] * finPitch + 5 * finPitch - 0.006
                if techInfo.numTrack == 6:
                    if techInfo.bprFlag == BprMode.BPR:
                ...
                if col[4] == 2:
                    uyLisd = col[4] * finPitch + 4 * finPitch - 0.02
                if col[4] == 4:
                    uyLisd = col[4] * finPitch + 5 * finPitch - 0.004
                if techInfo.numTrack == 6:
                    lyLisd = uyLisd - finPitch * (col[1] + col[3])
                elif techInfo.numTrack == 8:
                    lyLisd = uyLisd - finPitch * (col[1] + col[3] - 0.5)
                if techInfo.numTrack == 10 or techInfo.numTrack == 12:
                    lyLisd = uyLisd - finPitch * (col[1] + col[3])
                elif techInfo.numTrack == 11:
                    lyLisd = uyLisd - finPitch * (col[1] + col[3] - 0.5)
                else:
        ...
        for col in sortedColListNMOS:
            if col[4] == 6 and (col[0], 0) in LISD_connected_columns:
                continue
            if col[4] == 0 and (col[0], 1) in LISD_connected_columns:
                continue
            if col[4] == 6:
                lyLisd = (col[4]) * finPitch + 3 * finPitch + 0.006
            if col[4] == 0:
                lyLisd = (col[4]) * finPitch + finPitch
            xCoordLisd = xOffset + col[0] * gatePitch / 2
            if techInfo.numTrack == 6:
                if techInfo.bprFlag == BprMode.BPR:
                    lyLisd = finPitch / 2
        ...
        for col in sortedColListPMOS:
            ...
            xCoordLisd = xOffset + col[0] * gatePitch / 2
            if col[4] == 4:
                uyLisd = col[4] * finPitch + 5 * finPitch - 0.0065
            if col[4] == 2:
                uyLisd = col[4] * finPitch + 3 * finPitch + 0.0005
            if techInfo.numTrack == 6:
                if techInfo.bprFlag == BprMode.BPR:
                    lyLisd = uyLisd - finPitch / 2
                else:
                    # lyLisd = uyLisd - finPitch
                    pass
                uyLisd = lyLisd + finPitch * (col[1] - 0.25)
            elif techInfo.numTrack == 8:
                if techInfo.bprFlag == BprMode.BPR:
        ...
        for via in vias:
            ...
            if techInfo.numTrack == 6:
                ...
                # RT=10 --> Double height for 4T
                yOffset_via = yOffset - techInfo.metal2Pitch / 1000.0
            elif techInfo.numTrack == 14:
                # RT=14 --> Double height for 6T
                yOffset_via = yOffset - techInfo.metal2Pitch / 1000.0
