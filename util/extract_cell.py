...
def evaluate(conv_path, horizontal_pitch, cpp):
    ...
    return str(cell_width//(2*cpp) + 1), via_cnt, metal_length, cell_width, top_track_cnt
...
def main():
    if len(sys.argv) < 2:
        print("[INFO] Usage: python metrics.py <path_to_conv_files> <CPP> <path to csv files>")
        sys.exit(1)
    ...
    # extract the base name of the cell without the extension
    cell_name = os.path.basename(curr_cell_path).split(".")[0]

    # extract values from config file
    pc, vc, ml, cw, ttc = evaluate(curr_cell_path, 24, cpp)

    # add the data to a csv file
    if len(sys.argv) == 4:
        csv_path = sys.argv[3]
        print("         [INFO] Writing to csv: " + csv_path)
        with open(csv_path, "a+") as f:
            f.write(f"{cell_name}, {pc},{vc},{ml},{cw},{ttc}\n")
