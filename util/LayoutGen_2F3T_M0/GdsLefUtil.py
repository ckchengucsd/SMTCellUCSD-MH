    ...
    def GetMacroGdtStr(conv, cellName, techInfo, isUseMaxCellWidth, macroPinInfo):
        ...
        for inst in insts:
            ...
            if techInfo.multi_height_mode:
                ...
                for i in range(4):
                    ly_to_mos_type[math.ceil(i * single_row_height)] = placement_order[
                        i
                    ]
                ...
                if ly_to_mos_type[inst.ly] == "PMOS" and ly_to_cell_row[inst.ly] == 0:
                    ...
                    # PMOS @ row 1 (bottom)
                    uyActive = (
                        techInfo.cellHeight / 1000.0 - (yOffset_Active) - row_height
                    )
        ...
        for inst in insts:
            ...
            for i in range(xInst, xInst + inst.totalWidth, 2):
                isPgContact = 1
                if int(inst.ly) == 2 and ((3, i) in pinCols_PMOS):
                    isPgContact = 0
                elif int(inst.ly) == 3 and ((2, i) in pinCols_PMOS):
                    isPgContact = 0
                elif int(inst.ly) == 0 and ((5, i) in pinCols_NMOS):
                    isPgContact = 0
                elif int(inst.ly) == 5 and ((0, i) in pinCols_NMOS):
                    isPgContact = 0
                # isExtCol
                isExtCol = 0
        ...
        for col in sortedColListNMOS:
            ...
            if col[2]:  # isPgContact
                ...
                if col[4] == nmos_upper_plc:
                    lyLisd = (col[4]) * finPitch + finPitch * 2 + 0.018
                if techInfo.numTrack == 8:
                    if techInfo.bprFlag == BprMode.BPR:
                ...
                if col[4] == nmos_lower_plc:
                    lyLisd = (col[4]) * finPitch
                if col[4] == nmos_upper_plc:
                    lyLisd = (col[4]) * finPitch + finPitch * 2 + 0.008 - 0.012
                if techInfo.bprFlag == BprMode.BPR:
        ...
        for col in sortedColListPMOS:
            ...
            if col[2]:  # isPgContact
                ...
                if col[4] == pmos_lower_plc:
                    uyLisd = col[4] * finPitch + 4 * finPitch - 0.02 - 0.024
                if col[4] == pmos_upper_plc:
                    uyLisd = col[4] * finPitch + 5 * finPitch - 0.004 - 0.012
                if techInfo.numTrack == 8:
