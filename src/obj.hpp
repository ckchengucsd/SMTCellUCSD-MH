...
class Pin {
  ...
  */
public:
  // net related PinIO constants
  static std::string SOURCE_NET;
  static std::string SINK_NET;

  // pin related PinType constants
  static std::string SOURCE;
  static std::string DRAIN;
  static std::string GATE;
...
public:
  ...
  int getInstX() const { return instX_; }
  void setInstX(int x) { instX_ = x; }
  // genTestCase
  std::string getInstID() const { return instID_; }
...
private:
  ...
  int instWidth_;
  int instY_;
  int instX_; // for placement hint only
  int partitionGroup_;
  // genTestCase
  std::string instID_;
  int width_;
