      ...
      for (int row_idx = 0;
        ...
        for (int col_idx = 0;
          ...
          if (SMTCell::getVertex(metal, row, col)->getBackADJ()->ifValid()) {
            // MH FLAG: do not connnect the middle rows
      ...
      // Vertical Direction: consider rowwise constraint
        ...
        for (int row_idx = 0; row_idx <= SMTCell::get_h_metal_numTrackH(metal) -
          ...
          for (int col_idx = 0;
            ...
            // fmt::print("d     MAR : METAL {} ROW {} COL {} \n", metal, row,
            // col); fmt::print("vCoord: {} \n", vCoord->getVName());
            // SMTCell::getVertex((*vCoord))->dump();
...
void DesignRuleWriter::write_EOL_AGR_rule(
  ...
  // if (EOL_Parameter == 0) {
  if (EOL_M1_B_ADJ_Parameter == 0 && EOL_M1_B_Parameter == 0 && EOL_M2_R_ADJ_Parameter == 0 &&
      EOL_M2_R_Parameter == 0 && EOL_M3_B_ADJ_Parameter == 0 && EOL_M3_B_Parameter == 0 &&
      EOL_M4_R_ADJ_Parameter == 0 && EOL_M4_R_Parameter == 0) {
    ...
    //     EOL_M2_R_ADJ_Parameter, EOL_M2_R_Parameter, 84, EOL_M4_R_ADJ_Parameter,
    //     EOL_M4_R_Parameter, doublePowerRail, drlog);
    DesignRuleWriter::write_EOL_AGR_RL_tip_helper(
        EOL_M2_R_ADJ_Parameter, EOL_M2_R_Parameter, 0, EOL_M4_R_ADJ_Parameter,
        EOL_M4_R_Parameter, doublePowerRail, drlog);
    SMTCell::writeConstraint("\n");
        ...
        for (int col_idx = 0;
          ...
          if (vCoord_R->ifValid()) {
            ...
            while (distToPOICol <= realEOL) {
              ...
              // if (metal == 2) {fmt::print("distToPOICol: {} \n", distToPOICol);}
              if (!SMTCell::ifAssigned(tmp_str) && ((metal == 2 && distToPOICol != EOL_M2_R_SKIP_Parameter) || (metal == 4))) {
                // DEBUG MODE
                if (SMTCell::DEBUG_MODE_) {
                  fmt::print(drlog,
                             "d       => Adding Forbidden EOL RL : {} \n",
    ...
    if (SMTCell::ifHorzMetal(metal)) {
      for (int row_idx = 0;
        ...
        for (int col_idx = 0;
          ...
          // Triplet *vCoord_L = SMTCell::getVertex(metal, row, col)->getLeftADJ();
...
void DesignRuleWriter::write_VR_AGR_rule(float VR_M1M2_Parameter,
  ...
  if (VR_M1M2_Parameter == 0 && VR_M2M3_Parameter == 0 &&
    ...
    /* Apply via rule regardless nets */
    SMTCell::writeConstraint(fmt::format(
        ";VIA Rule for M2, VIA Rule is applied only for all vias\n"));
    fmt::print(drlog,
               ";VIA Rule for M2, VIA Rule is applied only for all vias\n");
    DesignRuleWriter::write_VR_AGR_M2_helper(VR_M2M3_Parameter, doublePowerRail,
                                             drlog);
    std::cout << "have been written.\n";
  }
}
...
void DesignRuleWriter::write_VR_AGR_M2_helper(float VR_Parameter,
                                              int doublePowerRail,
                                              FILE *drlog) {
  for (int metal = 2; metal <= 2; metal++) {
    for (int row_idx = 0; row_idx <= SMTCell::get_h_metal_numTrackH(metal) - 3;
         row_idx++) {
      for (int col_idx = 0;
           col_idx <= SMTCell::get_h_metal_numTrackV(metal) - 1; col_idx++) {
        ...
        if (SMTCell::getVertex((*vCoord))
          ...
          for (int newRow_idx = row_idx;
            ...
            for (int newCol_idx = col_idx;
              ...
              if (metal > 2 && SMTCell::ifVertMetal(metal) &&
                  SMTCell::ifSDCol_AGR(
                      1, SMTCell::getVertex((*neighborCoord))->getCol())) {
                if (SMTCell::DEBUG_MODE_) {
                  fmt::print(
          ...
          if (SMTCell::getVertex((*vCoord))->getUpADJ()->ifValid()) {
            ...
            for (int newRow_idx = row_idx;
              ...
              for (int newCol_idx = col_idx;
                ...
                std::shared_ptr<Triplet> neighborCoord = std::make_shared<Triplet>(*vCoord);;

                while (distCol > 0) {
                  distCol -= SMTCell::getMetalPitch(metal);
                  // free neighborCoord and get the right neighbor
                  neighborCoord = SMTCell::getVertex((*neighborCoord))->getRightADJ();
                  if (neighborCoord->ifValid() == false) {
                    if (SMTCell::DEBUG_MODE_) {
                      fmt::print(
                          drlog,
...
// Boundary Condition
/*
  Check if via enclosure exceed cell boundary, if so, prohibit via creation
*/
void DesignRuleWriter::write_VR_Boundary_rule(float M2_ENC_Parameter) {
  int metal = 3;
  int left_col = SMTCell::get_h_metal_col_by_idx(metal, 0);
  SMTCell::writeConstraint("; Left Boundary Condition\n");
  SMTCell::writeConstraint(fmt::format("(assert (and"));
  for (int udEdgeIndex = 0; udEdgeIndex < SMTCell::getUdEdgeCnt();
    ...
    if (fromMetal == 3 && toMetal == 4 && fromCol == left_col && toCol == left_col) {
      ...
      if (!SMTCell::ifAssigned(variable_name)) {
        ...
        SMTCell::writeConstraint(fmt::format(" (= {} false)", variable_name));
      }
    }
  }
  SMTCell::writeConstraint("))\n");
  SMTCell::writeConstraint("; Right Boundary Condition\n");
  for (int col_idx = 1; col_idx <= SMTCell::get_h_metal_numTrackV(metal) - 1; col_idx++) {
    // TODO: use real distance later
    // retrieve col
    int col = SMTCell::get_h_metal_col_by_idx(metal, col_idx);
    SMTCell::writeConstraint(fmt::format("(assert (ite (= COST_SIZE (_ bv{} {})) (and", col,
    ...
    for (int udEdgeIndex = 0; udEdgeIndex < SMTCell::getUdEdgeCnt();
      ...
      int toMetal = SMTCell::getUdEdge(udEdgeIndex)->getUdEdgeToVar()->metal_;
      // exclude M1 only edges
      if (fromMetal == 3 && toMetal == 4 && fromCol == col && toCol == col) {
        std::string variable_name = fmt::format(
          "M_{}_{}",
        SMTCell::getUdEdge(udEdgeIndex)->getUdEdgeFromVar()->getVName(),
        SMTCell::getUdEdge(udEdgeIndex)->getUdEdgeToVar()->getVName());
...
// Pin Access Rule
void DesignRuleWriter::write_pin_access_rule(int M1_MPO_Parameter,
                                             int MAR_Parameter,
                                             int EOL_Parameter, FILE *drlog) {
  DesignRuleWriter::write_pin_access_rule_helper(M1_MPO_Parameter, MAR_Parameter,
                                                 EOL_Parameter, drlog);
  ...
  SMTCell::writeConstraint("; 12-A. Each via must be connected by a upper metal\n");
  DesignRuleWriter::write_via_must_connect_upper_metal(drlog);
  SMTCell::writeConstraint("; (disabled) 12-A. Each via must be connected by a lower metal\n");
  // DesignRuleWriter::write_via_must_connect_lower_metal(drlog);
  SMTCell::writeConstraint("\n");
...
void DesignRuleWriter::write_pin_access_rule_helper(int M1_MPO_Parameter,
                                                    int MAR_Parameter,
                                                    int EOL_Parameter,
                                                    FILE *drlog) {
                ...
                if (SMTCell::getVirtualEdge(i)->getPinName() ==
                  ...
                  if (!SMTCell::ifAssigned(tmp_str_i)) {
                    ...
                    SMTCell::writeConstraint(fmt::format(
                        ...
                        "{} true)) ((_ at-least {})",
                        en.first, tmp_str_i, M1_MPO_Parameter));
                  } else if (SMTCell::ifAssignedTrue(tmp_str_i)) {
                    SMTCell::setVar_wo_cnt(tmp_str_i, 0);
                    SMTCell::writeConstraint(
                        fmt::format("(assert (ite (= N{}_M4_TRACK false) ((_ "
                                    "at-least {})",
                                    en.first, M1_MPO_Parameter));
                  }
                    ...
                    for (int marIdx = 0;
                      ...
                      for (int netIndex_j = 0;
                        ...
                        if (SMTCell::getNet(netIndex_j)->getNetID() !=
                          ...
                          //             sizeof(Triplet));
                          // Triplet *prev_left_vCoord = new Triplet(*vCoord_j);
                          ...
                          //             sizeof(Triplet));
                          // Triplet *prev_right_vCoord = new Triplet(*vCoord_j);
...
void DesignRuleWriter::write_via_must_connect_upper_metal(FILE *drlog) {
  for (int netIndex = 0; netIndex < SMTCell::getNetCnt(); netIndex++) {
...
void DesignRuleWriter::write_via_must_connect_lower_metal(FILE *drlog) {
  for (int netIndex = 0; netIndex < SMTCell::getNetCnt(); netIndex++) {
    for (int metal = 2; metal <= SMTCell::getNumMetalLayer() - 1; metal++) {
      for (int row_idx = 0;
           row_idx <= SMTCell::get_h_metal_numTrackH(metal) - 3; row_idx++) {
        for (int col_idx = 0;
          ...
          std::shared_ptr<Triplet> vCoord =
              std::make_shared<Triplet>(metal, row, col);
          // Triplet *vCoord_Down = SMTCell::getVertex((*vCoord))->getUpADJ();
          std::shared_ptr<Triplet> vCoord_Down =
              SMTCell::getVertex((*vCoord))->getDownADJ();

          // AGR FLAG: on M2, vCoord_Down could be null
          if (vCoord_Down->ifValid() == false) {
            continue;
          }
          ...
          if (SMTCell::ifHorzMetal(metal)) {
            // Front Vertex
            // potential bug, this should be vCoord_Down?
            if (SMTCell::getVertex((*vCoord_Down))->getFrontADJ()->ifValid()) {
              tmp_j = fmt::format(
                  "N{}_E_{}_{}", SMTCell::getNet(netIndex)->getNetID(),
                  SMTCell::getVertex((*vCoord_Down))->getFrontADJ()->getVName(),
                  vCoord_Down->getVName());
            } else {
              tmp_j = "null";
            }

            // Back Vertex
            if (SMTCell::getVertex((*vCoord_Down))->getBackADJ()->ifValid()) {
              tmp_k = fmt::format(
                  "N{}_E_{}_{}", SMTCell::getNet(netIndex)->getNetID(),
                  vCoord_Down->getVName(),
                  SMTCell::getVertex((*vCoord_Down))->getBackADJ()->getVName());
            } else {
              tmp_k = "null";
            }
          } else {
            // vertical direction
            // Left Vertex
            if (SMTCell::getVertex((*vCoord_Down))->getLeftADJ()->ifValid()) {
              tmp_j = fmt::format(
                  "N{}_E_{}_{}", SMTCell::getNet(netIndex)->getNetID(),
                  SMTCell::getVertex((*vCoord_Down))->getLeftADJ()->getVName(),
                  vCoord_Down->getVName());
            } else {
              tmp_j = "null";
            }
            ...
            // Right Vertex
            if (SMTCell::getVertex((*vCoord_Down))->getRightADJ()->ifValid()) {
              tmp_k = fmt::format(
                  "N{}_E_{}_{}", SMTCell::getNet(netIndex)->getNetID(),
                  vCoord_Down->getVName(),
                  SMTCell::getVertex((*vCoord_Down))->getRightADJ()->getVName());
            } else {
              tmp_k = "null";
            }
          }
