...
public:
  ...
  static void initMHInfo_helper_2T_4T(const std::string &config_path);

  static void initMHInfo_helper_3T(const std::string &config_path);

  // Placement: Fixed Cells
  static void setPlcHint(bool plcHint) { SMTCell::plcHint = plcHint; }

  static void setInstX(int instIdx, int x) {
    insts[instIdx]->setInstX(x);
    instPlacementCnt += 1;
  }

  static int getInstX(int instIdx) { return insts[instIdx]->getInstX(); }

  static bool getPlcHint() { return plcHint; }

  static int getInstPlacementCnt() { return instPlacementCnt; }
  ...
  static void writeCostHint(FILE *fp) {
    ...
    if (costHint > 0) {
      ...
      //            getBitLength_numTrackV());
      fmt::print(fp, "(assert (= COST_SIZE (_ bv{} {})))\n", costHint,
                 getBitLength_numTrackV());
    }
  }
  ...
  static void set_cell_config(int width, int height) {
    ...
    fmt::print("d     #     standardCellWidth = {}\n", standardCellWidth);
    fmt::print("d     #     standardCellHeight = {}\n", standardCellHeight);
  }
  ...
  static int get_h_metal_last_col(int metal) {
    return h_metal_col[metal][h_metal_col[metal].size() - 1];
  }
  ...
  // LISD Columns
  static void set_lisd_col(int site_idx, int col) {
    // check if site_idx is in the list, if not create a new list with key
    // site_idx
    // fmt::print("      Site {} : {}\n", site_idx, col);
    if (h_lisd_col.find(site_idx) == h_lisd_col.end()) {
      h_lisd_col[site_idx] = std::vector<int>();
    }
    h_lisd_col[site_idx].push_back(col);
  }

  static void is_lisd_col_empty() {
    if (h_lisd_col.empty()) {
      fmt::print("[WARNING] LISD Columns are empty!\n");
      // exit(1);
    }
  }

  static void clear_lisd_col() { h_lisd_col.clear(); }

  static void dump_h_lisd_col() {
    fmt::print("      Size of LISD Columns: {}\n", h_lisd_col.size());
    fmt::print("      LISD Columns: ");
    for (auto &it : h_lisd_col) {
      fmt::print("Site {} : ", it.first);
      for (auto &it2 : it.second) {
        fmt::print("{} ", it2);
      }
    }
  ...
  static bool is_lisd_col(int site_idx, int col) {
    // first check if site_idx is in the list
    if (h_lisd_col.find(site_idx) == h_lisd_col.end()) {
      return false;
    }
    // then check if col is in the list
    for (auto &it : h_lisd_col[site_idx]) {
      if (it == col) {
        return true;
      }
    }
  ...
  static int getNumFin() {
    if (numTrack_ == 2) {
      ...
      fmt::print("[ERROR] numTrack_ is not 2, 3, 4, 5, or 6");
      exit(-1);
    }
  };
  ...
  // MultiHeight
  static int getNumSite() { return SMTCell::num_site; }
  static int getNumClip() { return SMTCell::num_clip; }
  static std::string getOrderClip() { return SMTCell::order_clip; }
  static std::map<int, int> getNumRowPerClip() {
    return SMTCell::num_row_per_clip;
  }
  static int get_site_idx_by_row(int input_row) {
    // Check PMOS rows
    for (size_t site_idx = 0; site_idx < mh_pmos_placement_rows.size();
         ++site_idx) {
      const auto &row = mh_pmos_placement_rows[site_idx];
      for (int element : row) {
    ...
    // Check NMOS rows
    for (size_t site_idx = 0; site_idx < mh_nmos_placement_rows.size();
         ++site_idx) {
      const auto &row = mh_nmos_placement_rows[site_idx];
      for (int element : row) {
        if (element == input_row) {
          return site_idx;
        }
    ...
    return -1; // Return -1 if the input row is not found in any group
  }
  static int get_mos_pin_row_by_site_idx(int site_idx, int is_pmos) {
    if (is_pmos) {
      return mh_pmos_pin_rows[site_idx];
    } else {
      return mh_nmos_pin_rows[site_idx];
    }
    return -1;
  }
  static std::vector<int> get_mos_placement_rows_by_site_idx(int site_idx,
                                                             int is_pmos) {
    if (is_pmos) {
      return mh_pmos_placement_rows[site_idx];
    } else {
      return mh_nmos_placement_rows[site_idx];
    }
    return {};
  }
  static std::vector<int> get_mos_routing_rows_by_site_idx(int site_idx,
                                                           int is_pmos) {
    if (is_pmos) {
      return mh_pmos_routing_rows[site_idx];
    } else {
      return mh_nmos_routing_rows[site_idx];
    }
    return {};
  }
  static int get_routing_row_by_site_idx_and_metal_and_row_idx(int site_idx,
                                                               int metal,
                                                               int row_idx) {
    return SMTCell::get_h_metal_row_by_idx(metal,
                                           mh_routing_rows[site_idx][row_idx]);
  }
  static int get_gate_row_by_site_idx(int site_idx, int is_pmos) {
    if (is_pmos) {
      return mh_pmos_gate_rows[site_idx];
    } else {
      return mh_nmos_gate_rows[site_idx];
    }
    return -1;
  }
  static int get_mos_placeable_row_by_site_idx(int site_idx, int is_pmos) {
    if (is_pmos) {
      return mh_pmos_placeable_rows[site_idx];
    } else {
      return mh_nmos_placeable_rows[site_idx];
    }
    return -1;
  }
  // iterators
  static std::vector<int>::iterator placement_rows_begin(int site_idx) {
    return mh_placement_rows[site_idx].begin();
  }
  static std::vector<int>::iterator placement_rows_end(int site_idx) {
    return mh_placement_rows[site_idx].end();
  }
  static std::vector<int>::iterator routing_rows_begin(int site_idx) {
    return mh_routing_rows[site_idx].begin();
  }
  static std::vector<int>::iterator routing_rows_end(int site_idx) {
    return mh_routing_rows[site_idx].end();
  }
  static std::vector<std::vector<int>>::iterator pmos_placement_rows_begin() {
    return mh_pmos_placement_rows.begin();
  }
  static std::vector<std::vector<int>>::iterator pmos_placement_rows_end() {
    return mh_pmos_placement_rows.end();
  }
  static std::vector<std::vector<int>>::iterator nmos_placement_rows_begin() {
    return mh_nmos_placement_rows.begin();
  }
  static std::vector<std::vector<int>>::iterator nmos_placement_rows_end() {
    return mh_nmos_placement_rows.end();
  }
  static std::vector<int>::iterator pmos_placeable_rows_begin() {
    return mh_pmos_placeable_rows.begin();
  }
  static std::vector<int>::iterator pmos_placeable_rows_end() {
    return mh_pmos_placeable_rows.end();
  }
  static std::vector<int>::iterator nmos_placeable_rows_begin() {
    return mh_nmos_placeable_rows.begin();
  }
  static std::vector<int>::iterator nmos_placeable_rows_end() {
    return mh_nmos_placeable_rows.end();
  }
  static std::vector<std::vector<int>>::iterator pmos_routing_rows_begin() {
    return mh_pmos_routing_rows.begin();
  }
  static std::vector<std::vector<int>>::iterator pmos_routing_rows_end() {
    return mh_pmos_routing_rows.end();
  }
  static std::vector<int>::iterator pmos_routing_rows_begin(int site_idx) {
    return mh_pmos_routing_rows[site_idx].begin();
  }
  static std::vector<int>::iterator pmos_routing_rows_end(int site_idx) {
    return mh_pmos_routing_rows[site_idx].end();
  }
  static std::vector<std::vector<int>>::iterator nmos_routing_rows_begin() {
    return mh_nmos_routing_rows.begin();
  }
  static std::vector<std::vector<int>>::iterator nmos_routing_rows_end() {
    return mh_nmos_routing_rows.end();
  }
  static std::vector<int>::iterator nmos_routing_rows_begin(int site_idx) {
    return mh_nmos_routing_rows[site_idx].begin();
  }
  static std::vector<int>::iterator nmos_routing_rows_end(int site_idx) {
    return mh_nmos_routing_rows[site_idx].end();
  }
  static std::vector<int>::iterator pmos_pin_rows_begin() {
    return mh_pmos_pin_rows.begin();
  }
  static std::vector<int>::iterator pmos_pin_rows_end() {
    return mh_pmos_pin_rows.end();
  }
  static std::vector<int>::iterator nmos_pin_rows_begin() {
    return mh_nmos_pin_rows.begin();
  }
  static std::vector<int>::iterator nmos_pin_rows_end() {
    return mh_nmos_pin_rows.end();
  }
  static std::vector<int>::iterator pmos_gate_rows_begin() {
    return mh_pmos_gate_rows.begin();
  }
  static std::vector<int>::iterator pmos_gate_rows_end() {
    return mh_pmos_gate_rows.end();
  }
  static std::vector<int>::iterator nmos_gate_rows_begin() {
    return mh_nmos_gate_rows.begin();
  }
  static std::vector<int>::iterator nmos_gate_rows_end() {
    return mh_nmos_gate_rows.end();
  }
  ...
  // }
  static bool ifVirtualEdge(std::shared_ptr<Triplet> vCoord,
              std::string pinName) {
    for (const auto &ve : virtualEdges) {
      if (ve->getVCoord() == vCoord && ve->getPinName() == pinName) {
        return true;
      }
    }
    return false;
...
private:
  ...
  static int num_clip;
  static int num_site;
  static std::string order_clip;
  static std::map<int, int> num_row_per_clip;
  static std::map<int, std::vector<int>> mh_placement_rows;
  static std::map<int, std::vector<int>> mh_routing_rows;
  static std::vector<std::vector<int>> mh_pmos_placement_rows;
  static std::vector<std::vector<int>> mh_nmos_placement_rows;
  static std::vector<int> mh_pmos_placeable_rows;
  static std::vector<int> mh_nmos_placeable_rows;
  static std::vector<std::vector<int>> mh_pmos_routing_rows;
  static std::vector<std::vector<int>> mh_nmos_routing_rows;
  static std::vector<int> mh_pmos_pin_rows;
  static std::vector<int> mh_nmos_pin_rows;
  static std::vector<int> mh_pmos_gate_rows;
  static std::vector<int> mh_nmos_gate_rows;
  ...
  // store all lisd columns
  // static std::vector<int> h_lisd_col;
  static std::map<int, std::vector<int>> h_lisd_col;
