      ...
      while (row <= SMTCell::getCellHeight()) {
        ...
        while (upCol <= SMTCell::getCellWidth()) {
          ...
          if (std::find(temp_cols.begin(), temp_cols.end(), upCol) ==
              temp_cols.end()) {
            // offset flag: do not add the edge between 0 and offset position
            if (lowVertOffset > 0 && uppVertOffset == 0 && upCol == 0) {
              // continue;
            } else {
        ...
        while (lowCol <= SMTCell::getCellWidth()) {
          ...
          if (std::find(temp_cols.begin(), temp_cols.end(), lowCol) ==
              temp_cols.end()) {
            // offset flag: do not add the edge between 0 and offset position
            if (uppVertOffset > 0 && lowVertOffset == 0 && lowCol == 0) {
              // continue;
            } else {
      ...
      for (int col_idx = 0;
        ...
        // # Odd Layers ==> Vertical
          ...
          if (udEdgeTerm2_p->ifValid()) {
            ...
            // else if (metal == 1 &&
            //            ((SMTCell::getNumTrack() == 4 && row_idx == 3) ||
            //             (SMTCell::getNumTrack() == 5 && row_idx == 4) ||
            //             (SMTCell::getNumTrack() == 6 && row_idx == 5))) {
            //   // metal 1 respects power rail, no back edge through or between
            //   // rows for example, if we have 12 rows, row 5 cannot connect
            //   to
            //   // row 6 Need to check if it is middle row
            //   // TODO: based on super cut node setting, limit the connection
            //   // do nothing
            //   // fmt::print("Skip Back Edge at metal {} row {}\n", metal,
            //   // row_idx);
            // }
            else {
              // SMTCell::addUdEdge(new UdEdge(udEdgeIndex, udEdgeTerm1_p,
      ...
      for (int col_idx = 0;
        ...
        if (EXT_Parameter == 0) {
          ...
          if (SMTCell::getNumTrack() == 2) {
            if (row_idx == 0 || row_idx == 1 || row_idx == 2 || row_idx == 3) {
              SMTCell::addBoundaryVertex(
                  std::make_shared<Triplet>(metal, row, col));
            }
          } else if (SMTCell::getNumTrack() == 3) {
            // if (row_idx == 1 || row_idx == 4) {
            if (row_idx == 0 || row_idx == 2 || row_idx == 3 || row_idx == 5) {
              SMTCell::addBoundaryVertex(
                  std::make_shared<Triplet>(metal, row, col));
            }
          } else if (SMTCell::getNumTrack() == 4) {
            // boundary vertices at row 1, row 2, row 6, row 7
            if (row_idx == 1 || row_idx == 2 ||
                row_idx == SMTCell::get_h_metal_numTrackH(metal) - 5 ||
    ...
    if (SMTCell::ifPinInst(pinIdx) &&
      ...
      if (SMTCell::ifPinInstPMOS(pinIdx)) {
        ...
        for (int site_idx = 0; site_idx < SMTCell::getNumSite(); site_idx++) {
          for (auto row_it = SMTCell::pmos_routing_rows_begin(site_idx);
               row_it < SMTCell::pmos_routing_rows_end(site_idx); row_it++) {
            for (int col_idx = 0;
      ...
      // NMOS
        ...
        int metal = 1;
        for (int site_idx = 0; site_idx < SMTCell::getNumSite(); site_idx++) {
          for (auto row_it = SMTCell::nmos_routing_rows_begin(site_idx);
               row_it < SMTCell::nmos_routing_rows_end(site_idx); row_it++) {
            for (int col_idx = 0;
                 col_idx <= SMTCell::get_h_metal_numTrackV(metal) - 1;
                 col_idx++) {
              int row_idx = *row_it;
